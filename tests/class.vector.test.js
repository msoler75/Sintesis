...
test('Vector-class-basic-1', () => {
    expect(createVectorWithSizes([3,2], -1).toString()).toContainText('[[-1, -1], [-1, -1], [-1, -1]]')
    ...
    expect(v.toString()).toContainText('[10, 20, 0, 0, [50, 60]]')
...
test('Vector-class-basic-2', () => {
    ...
    v.setVariable(3, m)
    expect(v.toString()).toContainText('[10, 20, -1, {j: 1, p: 2}]')

    v.setValue(5, 55)
    expect(v.toString()).toContainText('[10, 20, -1, {j: 1, p: 2}, -1, 55]')
})
...
test('Vector-class-delete', () => {
    ...
    v.delete(4)
    v.delete(2)
    expect(v.toString()).toContainText('[10, 20, -1, 40]')
    v.delete(3)
    expect(v.toString()).toContainText('[10, 20, -1]')
})