...
class SymbolTable {
  constructor(function_or_class) {
    ...
    if (function_or_class instanceof Function)
      this.function =
        function_or_class; // para señalar que estamos dentro de un nuevo contexto de función
    else if (function_or_class) this.class = function_or_class; // para señalar que estamos dentro de un nuevo contexto de instancia clase
  }
  ...
  pushStack(instance) {
    ...
    const m2 = {};
    if (!this.memory) throw new Error("Symbols not found");
    if (this.memory.length == 0) throw new Error("No Symbol stacks");
    ...
    keys.forEach((key) => {
      if (instance && m[key]._variable instanceof RefClass) {
        ...
        m2[key] = new MemoryRef(
          m[key]._variable &&
          (m[key]._variable instanceof Function ||
            m[key]._variable instanceof Class)
            ? m[key]._variable
            : undefined
        );
    });
  ...
  popStack() {
    if (this.memory.length <= 1) throw new Error("popStack invalid");
    this.memory.pop();
  }
  ...
  getRef(id) {
    ...
    if (r instanceof RefClass) return r.classInstance.symbolTable.getRef(id);
    return r;
  }
...
class SymbolFinder {
  ...
  static findSymbolContext(ctx, id) {
    ctx = this.findTable(ctx);
    while (ctx) {
      if (ctx.symbolTable.hasSymbol(id)) return ctx;
      if (ctx.symbolTable.class && ctx.symbolTable.class.superClass) {
        let ctxClass = ctx;
        while (ctxClass) {
          if (!ctxClass.symbolTable) throw new Error("SymbolTable not found");
          if (ctxClass.symbolTable.hasSymbol(id)) return ctxClass;
          ctxClass = ctxClass.symbolTable.class.superClass
            ? ctxClass.symbolTable.class.superClass.context
            : null;
        }
      }
  ...
  static visitChildren(ctx, fn) {
    ...
    if (ctx && ctx.children && ctx.children.length)
      ctx.children.forEach((item) => SymbolFinder.visitChildren(item, fn));
  }
  ...
  static pushStack(ctx, instance) {
    ...
    if (instance) {
      let inst = instance;
      while (inst) {
        if (!inst.class) throw new Error("Not Class Found in instance");
  ...
  static popStack(ctx, instance) {
    ctx = this.findTable(ctx);
    if (instance) ctx.symbolTable.popStack();
    else
      SymbolFinder.visitChildren(ctx, function (ctx) {
        if (ctx && ctx.symbolTable) {
  ...
  static clearValues(ctx) {
    ...
    for (const symbol of keys) {
      if (["___instance", "___attributes", "___methods", "___super"]) continue;
      else if (key === "___result") symbols[symbol] = undefined;
      else if (key === "___ended") symbols[symbol] = false;
      else {
  ...
  static getReturnValue(ctx) {
    ctx = this.findTable(ctx);
    while (ctx) {
      if (ctx.symbolTable.function) return ctx.symbolTable.getReturnValue();
      ctx = this.findTable(ctx.parentCtx);
    }
  }
  ...
  static functionHasEnded(ctx) {
    ctx = this.findTable(ctx);
    while (ctx) {
      if (ctx.symbolTable.function) return ctx.symbolTable.functionHasEnded();
      ctx = this.findTable(ctx.parentCtx);
    }
  }
  ...
  static getFuncContext(ctx) {
    ctx = this.findTable(ctx);
    while (ctx) {
      if (ctx.symbolTable.function) return ctx;
      ctx = this.findTable(ctx.parentCtx);
    }
    return null;
  }
  ...
  static getClassContext(ctx) {
    ctx = this.findTable(ctx);
    while (ctx) {
      if (ctx.symbolTable.class) return ctx;
      ctx = this.findTable(ctx.parentCtx);
    }
    return null;
  }
  ...
  static canAccess(memoryref, ctxFrom) {
    if (memoryref && memoryref._variable instanceof RefClass) {
      ...
      if (visibility == "private")
        return (
          ctx && ctx.symbolTable.class.name === rc._value.classInstance.name
        );
      // protected:
      return (
        ctx &&
        ctx.symbolTable.class.isDescendantOf(rc._value.classInstance.name)
      );
    }
    if (
      memoryref &&
      memoryref._variable instanceof Instance &&
      memoryref._index
    ) {
          ...
          Object.keys(m0)
            .map(
              (id) =>
                id +
                ": {" +
                ctx.symbolTable.memory
                  .map((m) => printValueOf(m[id]))
                  .join(", ") +
                "}"
            )
            .join(", ") +
