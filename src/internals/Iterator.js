...
class Iterator {
  ...
  get current() {
    ...
    switch (typeof c) {
      case "string":
        return c.charAt(this.index);
      case "object":
        if (c instanceof MemoryRef) c = c.variable;
        if (c instanceof List) return c._value[this.index];
        if (c instanceof Dictionary) return c._value[this.index];
        if (Array.isArray(c)) return c[this.index];
    }
    let coll = valueOf(this._collection);
    // TO-DO: getValueFrom ?
    return Array.isArray(coll)
      ? coll[this.index]
      : coll instanceof List
      ? coll.getValueFrom([this.index])
      : typeof coll === "string"
      ? coll.charAt(this.index)
      : coll[this.index];
  }
}
