...
export default class SintesisEval extends SintesisParserVisitor {
  ...
  // Visit a parse tree produced by SintesisParser#expMemberIndex.
   visitExpMemberIndex(ctx) {
    ...
    if (!ref && ctx.args)
      throw new SintesisError(ctx.idx, index + " no es un método válido");
    ...
    if (ref._variable instanceof List) {
      try {
        ...
        throw new SintesisError(ctx.idx, err);
      }
    }

    // comprobamos el tipo
    if (!(ref.variable instanceof Function))
      throw new SintesisError(ctx.mem, `${index} no se ha encontrado`);
  ...
  /*visitExpList(ctx) {
    const funcname = ctx.fn.text;
    const args =     this.visit(ctx.args).values;
    if (!(funcname in Array))
      throw new SintesisError(ctx.fn, "No existe esta método");
    return Array[funcname].apply(this, args);
  }*/
   ...
   visitArguments(ctx) {
    ...
    if (ctx.children.length > 2)
      for (let i = 1; i < ctx.children.length; i++)
        if (i % 2 == 1) ctx.values.push( this.visit(ctx.children[i]));
    return ctx;
  }
  ...
  // Visit a parse tree produced by SintesisParser#expParenthesis.
   visitExpJavascript(ctx) {
    ...
    var evaluated = true; // safeEval(code, context);
