...
class SintesisSymbolParser extends SintesisParserVisitor {
    ...
    // Visit a parse tree produced by SintesisParser#identifier.
        ...
        const id = getId(ctx)
        if (this.createIdentifierAlways)
            ...
            if (!memoryref || (memoryref._variable instanceof Error))
                if (this.createIdentifierIfNotFound)
                    SymbolFinder.addSymbol(ctx, id)
    ...
    // Visit a parse tree produced by SintesisParser#classDeclaration.
        ...
        const methodList = ctx.methods ? ctx.methods.children.map(x => new Function(getId(x.id), x, ctx)) : {}
        let extendingWithNoDefaultConstructor = extend && !extendedCls.hasDefaultConstructor()
        let methods = {}
        let numconstruc = 0
        for (let i = 0; i < methodList.length; i++) {
            ...
            if (extendingWithNoDefaultConstructor && isConstructor) {
                ...
                const firstStatement = method.context.children.find(x => x.constructor.name === 'FunctionBodyContext').children[0].children[1]
                ...
                if (!callingSuper)
                    throw new SintesisError(firstStatement, 'Debe llamar al método constructor de la clase padre')
        ...
        for (const id in cls.methods)
            if (cls.methods[id].context)
                     await this.visit(cls.methods[id].context)
    }
    ...
    // Visit a parse tree produced by SintesisParser#forClassic.
        ...
        SymbolFinder.createTable(ctx)
        if (ctx.pre)
                 await this.visit(ctx.pre)
        if (ctx.exp)
                 await this.visit(ctx.exp)
        if (ctx.stmt)
                 await this.visit(ctx.stmt)
        if (ctx.post)
                 await this.visit(ctx.post)
    }
