...
export default class SintesisEval extends SintesisParserVisitor {
  ...
  // Visit a parse tree produced by SintesisParser#expNew.
    ...
    if (!memoryref) throw new SintesisError(ctx.id, this.t("no existe la clase '%s'", id));
    ...
    if (!constructor)
      throw new SintesisError(
        ctx.args,
        this.t("no existe un método constructor") + " " +
          values.length
            ? (values.length==1?
              this.t("con 1 parámetro"):
              this.t("con %d parámetros", values.length)
            )
            : this.t("sin parámetros")
          );
 ...
 async callToFunction(memoryref, ctxArgs) {
    ...
    if (params.length !== values.length)
      throw new SintesisError(
        ctxArgs,
        (fn.isConstructor ? this.t("el constructor") : this.t("la función '%s'", fn.name)) +
        " " +
          (params.length
            ? (
              params.length==1?
              this.t("requiere 1 argumento"):
  ...
  // Visit a parse tree produced by SintesisParser#expMemberIndex.
    ...
    if (!ref && ctx.args)
      throw new SintesisError(ctx.idx, this.t("'%s' no es un método válido", index));
    ...
    if (!(ref.variable instanceof Function))
      throw new SintesisError(ctx.mem, this.t("'%s' no se ha encontrado", index));
  ...
  // Visit a parse tree produced by SintesisParser#expMemberFunc.
    ...
    if (!memoryref) throw new SintesisError(ctx.mem, this.t("función no encontrada"));
  ...
  // Visit a parse tree produced by SintesisParser#listIndex.
    ...
    const key =     await this.visit(ctx.key);
  ...
  // Visit a parse tree produced by SintesisParser#expAssignmentOperator.
    ...
    if (!memoryref) throw new SintesisError(ctx.dest, this.t("símbolo no encontrado"));
    if (!(memoryref instanceof MemoryRef))
      throw new SintesisError(ctx.dest, this.t("operador izquierdo no es válido"));
  ...
  // Visit a parse tree produced by SintesisParser#expBasicFunction.
    ...
    switch (fn) {
      ...
      case "NumberOfContext":
        if (args.length < 1)
          throw new SintesisError(ctx.args, this.t("debe especificar un argumento"));
        if ((t0 !== "string") & (t0 !== "object"))
          throw new SintesisError(ctx.args.children[1], this.t("tipo incorrecto"));
      ...
      case "IndexOfContext":
        if (args.length < 2)
          throw new SintesisError(ctx.args, this.t("debe especificar dos argumentos"));
      ...
      case "ConvertContext":
        if (args.length < 2)
          throw new SintesisError(ctx.args, this.t("debe especificar dos argumentos"));
        if (t1 !== "string")
          throw new SintesisError(ctx.args.children[3], this.t("tipo incorrecto"));
        ...
        if (paramNumber.includes(a1.toLowerCase())) {
          ...
          if (t0 === "number") return "" + a0;
        } else throw new SintesisError(ctx.args.children[3], this.t("tipo incorrecto"));
        throw new SintesisError(ctx.args.children[1], this.t("no se pudo convertir"));
      ...
      case "SubContext":
        ...
        if (end && typeof end !== "number")
          throw new SintesisError(ctx.args.children[5], this.t("tipo incorrecto"));
  ...
  // Visit a parse tree produced by SintesisParser#expSuper.
    ...
    if (!class_ctx) throw new SintesisError(ctx, this.t("referencia fuera de clase"));
    if (!class_ctx.symbolTable.class.superClass)
      throw new SintesisError(ctx, this.t("no existe clase padre de esta clase"));
